...
test('DMC direct DAC writes mask to seven bits and delta output does not overflow', () => {
  ...
  assert.equal(output(bare), 127); const pcm = bare.drainSamples(); assert.ok(pcm[0] > 0 && pcm[1] > 0 && pcm[0] !== pcm[1], 'fixed DAC level passes through a transient filter response');

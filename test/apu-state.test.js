...
test('APU snapshots accept offset views and reject invalid field values atomically', () => {
  ...
  for (const corrupt of [
    ...
    bytes => { bytes[46] = 128; },
    bytes => { bytes[47] = 2; },
    bytes => { new DataView(bytes.buffer).setUint16(24, 0x800, true); },
    bytes => { bytes[55] = 254; },
    bytes => { bytes[12] = 8; },
    bytes => { bytes[36] = 2; },
    bytes => { new DataView(bytes.buffer).setUint32(41, 1789773, true); },
    bytes => { new DataView(bytes.buffer).setUint16(57, 29830, true); },

...
test('WASM Bandai mappers 70 and 152 switch lower PRG and CHR banks', async () => {
  for (const mapper of [70, 152]) {
    const { bytes, start } = rom(mapper, 8, 8);
    const code = [0xad, 0, 0x80, 0x85, 0, 0xa9, 0xa1, 0x8d, 0, 0x80,
      0xad, 0, 0x80, 0x85, 1, 0xad, 0, 0xc0, 0x85, 2, 0x4c, 0x14, 0x81];
    const chrStart = start + 8 * 0x4000;
    for (let bank = 0; bank < 16; bank++) bytes.fill(0x40 + bank, chrStart + bank * 0x1000, chrStart + (bank + 1) * 0x1000);
    bytes.set(code, start + 7 * 0x4000 + 0x100); vector(bytes, start, 8, 0xc100);
    const js = new Nes(bytes), core = await WasmCore.from(binary);
    js.reset(); core.loadRom(bytes); core.reset(); js.step(100); core.step(100);
    assert.deepEqual([0, 1, 2].map(i => core.exports.ramRead(i)), [0x30, 0x32, 0x37]);
    assert.deepEqual([core.exports.chrRead(0), core.exports.chrRead(0x1000)], [0x42, 0x43]);
    assert.deepEqual([core.programCounter, core.cycleCount], [js.cpu.pc, js.cycleCount]); assert.equal(core.exports.unknownOpcodeCount(), 0);
  }
});

test('WASM mapper 78 switches lower PRG, CHR and single-screen mirroring', async () => {
  const { bytes, start } = rom(78, 8, 8);
  const code = [0xad, 0, 0x80, 0x85, 0, 0xa9, 0x33, 0x8d, 0, 0x80,
    0xad, 0, 0x80, 0x85, 1, 0xad, 0, 0xc0, 0x85, 2, 0x4c, 0x14, 0x81];
  const chrStart = start + 8 * 0x4000;
  for (let bank = 0; bank < 16; bank++) bytes.fill(0x40 + bank, chrStart + bank * 0x1000, chrStart + (bank + 1) * 0x1000);
  bytes.set(code, start + 7 * 0x4000 + 0x100); vector(bytes, start, 8, 0xc100);
  const js = new Nes(bytes), core = await WasmCore.from(binary);
  js.reset(); core.loadRom(bytes); core.reset(); js.step(100); core.step(100);
  assert.deepEqual([0, 1, 2].map(i => core.exports.ramRead(i)), [0x30, 0x33, 0x37]);
  assert.deepEqual([core.exports.chrRead(0), core.exports.chrRead(0x1000)], [0x46, 0x47]);
  assert.deepEqual([core.programCounter, core.cycleCount], [js.cpu.pc, js.cycleCount]); assert.equal(core.exports.unknownOpcodeCount(), 0);
});

test('WASM mapper 89 decodes PRG and CHR register bits', async () => {
  const { bytes, start } = rom(89, 8, 16);
  const code = [0xad, 0, 0x80, 0x85, 0, 0xa9, 0xd2, 0x8d, 0, 0x80,
    0xad, 0, 0x80, 0x85, 1, 0xad, 0, 0xc0, 0x85, 2, 0x4c, 0x14, 0x81];
  const chrStart = start + 8 * 0x4000;
  for (let bank = 0; bank < 32; bank++) bytes.fill(0x40 + bank, chrStart + bank * 0x1000, chrStart + (bank + 1) * 0x1000);
  bytes.set(code, start + 7 * 0x4000 + 0x100); vector(bytes, start, 8, 0xc100);
  const js = new Nes(bytes), core = await WasmCore.from(binary);
  js.reset(); core.loadRom(bytes); core.reset(); js.step(100); core.step(100);
  assert.deepEqual([0, 1, 2].map(i => core.exports.ramRead(i)), [0x30, 0x35, 0x37]);
  assert.deepEqual([core.exports.chrRead(0), core.exports.chrRead(0x1000)], [0x54, 0x55]);
  assert.deepEqual([core.programCounter, core.cycleCount], [js.cpu.pc, js.cycleCount]); assert.equal(core.exports.unknownOpcodeCount(), 0);
});
...
test('WASM mapper 38 switches 32KB PRG and 8KB CHR from $7000', async () => {
  const { bytes, start } = rom(38, 8, 8);
  const chrStart = start + 8 * 0x4000;
  for (let bank = 0; bank < 8; bank++) bytes.fill(0x40 + bank, chrStart + bank * 0x2000, chrStart + (bank + 1) * 0x2000);
  const code = [0xad, 0, 0x80, 0x85, 0, 0xa9, 0x0d, 0x8d, 0, 0x70,
    0xad, 0, 0x80, 0x85, 1, 0xad, 0, 0xc0, 0x85, 2, 0x4c, 0x14, 0x81];
  for (let bank = 0; bank < 8; bank++) bytes.set(code, start + bank * 0x4000 + 0x100);
  bytes.set([0, 0x81], start + 0x8000 - 4);
  const js = new Nes(bytes), core = await WasmCore.from(binary);
  js.reset(); core.loadRom(bytes); core.reset(); js.step(100); core.step(100);
  assert.deepEqual([0, 1, 2].map(i => core.exports.ramRead(i)), [0x30, 0x32, 0x33]);
  assert.deepEqual([core.exports.chrRead(0), core.exports.chrRead(0x1000)], [0x43, 0x43]);
  assert.deepEqual([core.programCounter, core.cycleCount], [js.cpu.pc, js.cycleCount]); assert.equal(core.exports.unknownOpcodeCount(), 0);
});

...
test('mapper 225 encodes PRG and CHR banks from address writes', () => { const nes=new Nes(image(225,16,16)); nes.write(0x8123,0); assert.deepEqual(prgPair(nes),[4,5]); assert.deepEqual(chrPair(nes),[0x40,0x41]); nes.write(0xc123,0); assert.deepEqual(prgPair(nes),[4,5]); assert.equal(nes.cartridge.mirroring,'vertical'); const state=nes.saveState(); nes.write(0x8000,0); nes.loadState(state); assert.deepEqual(prgPair(nes),[4,5]); });

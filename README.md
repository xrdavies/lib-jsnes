...
implement a full audio mixer; it drops scheduling backlog above 200 ms to bound
latency. Arrow keys, Z/X, Enter and Shift control player 1.
...
Full-system tests cover all 30 supported mappers with CHR ROM and CHR RAM,
...
The current mapper layer supports NROM (0), MMC1 (1), UxROM (2), CNROM (3), MMC3 bank switching (4), AxROM (7), MMC2 (9), MMC4 (10), Color Dreams (11), CPROM (13), mapper 15, Irem G-101 (32), BNROM (34), mapper 38, GxROM (66), Bandai 74161/7432 (70), Camerica (71), mapper 78, mapper 79, mapper 87, mapper 89, mapper 94, mapper 113, mapper 140, Bandai 74161/7432 with switchable mirroring (152), mapper 177, mapper 180, mapper 225, mapper 240, and mapper 241. MMC3 IRQ counting follows filtered PPU A12 rises; advanced mapper variants and revision-specific edge behavior remain in progress.
...
Its cartridge path accepts iNES 1.0 NROM, MMC1, UxROM, CNROM, MMC3, AxROM, MMC2, MMC4, Color Dreams, CPROM, mapper 15, Irem G-101, BNROM, mapper 38, GxROM, Bandai 74161/7432, Camerica, mapper 78, mapper 89, mapper 94, mapper 180, and mappers 79, 87, 113, 140, 152, 177, 225, 240 and 241 (mappers 0, 1, 2, 3, 4, 7, 9, 10, 11, 13, 15, 32, 34, 38, 66, 70, 71, 78, 79, 87, 89, 94, 113, 140, 152, 177, 180, 225, 240 and 241), including
...
1 KiB for MMC3 and Irem G-101, 4 KiB for MMC1, or 8 KiB for other supported mappers. Smaller or
...
BNROM (mapper 34) switches a complete 32 KiB PRG window from writes at
`$8000–$FFFF`. Both cores wrap selectors across smaller images and retain the
header mirroring; CHR RAM or ROM uses the normal unbanked 8 KiB window. This
covers the common BNROM board without claiming support for mapper-34 NINA
variants with different register wiring.

Mapper 38 uses writes at `$7000–$7FFF` to select a 32 KiB PRG bank from bits
0–1 and an 8 KiB CHR bank from bits 2–3; other CPU writes retain normal behavior.

Color Dreams (mapper 11) uses CPU writes at `$8000–$FFFF` to select a 32 KiB
...
Irem G-101 (mapper 32) provides two switchable 8 KiB PRG registers and a mode
bit that swaps the fixed and switchable windows. Eight CPU-selectable 1 KiB CHR
pages are mapped at `$0000–$1FFF`; `$9000` also selects horizontal or vertical
mirroring. The four-screen header mode remains fixed when present.

Mapper 180 uses a fixed 16 KiB PRG window at `$8000–$BFFF` and a switchable
16 KiB window at `$C000–$FFFF`. Writes at `$8000–$FFFF` select the upper bank;
selectors wrap to the available PRG image size.

Bandai mappers 70 and 152 use a switchable 16 KiB PRG window at `$8000–$BFFF`,
the final PRG bank fixed at `$C000–$FFFF`, and a switchable 8 KiB CHR bank.
Mapper 152 also selects single-screen mirroring from bit 7 of the register;
mapper 70 retains the header mirroring.

Mapper 78 uses a switchable 16 KiB PRG window at `$8000–$BFFF`, the final PRG
bank fixed at `$C000–$FFFF`, and a switchable 8 KiB CHR bank. Register bit 3
selects the lower or upper single-screen nametable.

Mapper 89 uses the same 16 KiB lower and fixed-final PRG layout. Register bits
4–6 select PRG, bits 0–2 and 7 select the 8 KiB CHR bank, and bit 3 selects
single-screen mirroring.

Mapper 94 uses a switchable 16 KiB PRG window at `$8000–$BFFF` and fixes the
final bank at `$C000–$FFFF`; register bits 2–5 select the lower bank.

Mapper 240 uses one register at `$4020–$5FFF`; its upper nibble selects a
32 KiB PRG bank and its lower nibble selects an 8 KiB CHR bank. Selectors wrap
to the available image size.
...
upper window. Both use two PPU-latched 4 KiB CHR windows. Accesses in
`$0FD0–$0FDF`/`$0FE0–$0FEF` and `$1FD0–$1FDF`/`$1FE0–$1FEF` select the
corresponding bank pair, and `$F000–$FFFF` selects vertical or horizontal
mirroring. Mapper variants with different wiring are not claimed.

Camerica mapper 71 switches the lower 16 KiB PRG window from cartridge writes
outside `$9000–$9FFF` and fixes the upper window to the final PRG bank. Writes
in `$9000–$9FFF` select single-screen mirroring from bit 4; before such a write,
the header mirroring remains active. Board variants with different register
wiring are not claimed.

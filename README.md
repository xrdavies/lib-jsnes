...
| `dmc_dma_during_read4/` | Requires internal-register bus-conflict behavior; runner times out |
...
The separate `dmc_dma_during_read4` ROMs exercise CPU internal-register conflicts
when DMC reads overlap `$2007`/`$4016`; those board-level conflicts remain outside
the current bus model and are intentionally reported as timeouts by the runner.

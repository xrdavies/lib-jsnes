...
export class Nes implements CpuBus { readonly controller1 = new Controller(); readonly ppu = new Ppu(); readonly controller2 = new Controller(); readonly rom: RomImage; readonly frame = new Uint32Array(256 * 240); readonly cpu: Cpu6502; private readonly ram = new Uint8Array(0x800); private cycles = 0; private dmaStall = 0; private bank = 0;
  constructor(image: ArrayBuffer | Uint8Array) { this.rom=parseRom(image); this.ppu.setMirroring(this.rom.mirroring); this.cpu=new Cpu6502(this); }
  ...
  reset(){this.ram.fill(0); this.bank=0; this.dmaStall=0; this.frame.fill(0xff000000); this.cpu.reset(); this.cycles=0;}

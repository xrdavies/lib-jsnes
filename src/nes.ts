...
export class Nes implements CpuBus {
  ...
  step(cycles=1){if(!Number.isInteger(cycles)||cycles<1)throw new RangeError('cycles must be a positive integer'); const target=this.cycles+cycles; while(this.cycles<target){if(this.dmaStall){const used=Math.min(this.dmaStall,target-this.cycles); this.dmaStall-=used; this.cycles+=used; this.ppu.step(used*3); continue;} const used=this.cpu.step(); this.cycles+=used; this.ppu.step(used*3);}}

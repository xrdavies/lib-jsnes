...
export class Nes implements CpuBus { readonly controller1 = new Controller(); readonly controller2 = new Controller(); readonly rom: RomImage; readonly frame = new Uint32Array(256 * 240); readonly cpu: Cpu6502; private readonly ram = new Uint8Array(0x800); private cycles = 0; private bank = 0;
  constructor(image: ArrayBuffer | Uint8Array) { this.rom=parseRom(image); this.cpu=new Cpu6502(this); }
  read(address:number){address&=0xffff; if(address<0x2000)return this.ram[address&0x7ff]; if(address===0x4016)return this.controller1.read(); if(address===0x4017)return this.controller2.read(); if(address>=0x8000){ if(this.rom.mapper===2){const bankSize=0x4000; const bankCount=this.rom.prgRom.length/bankSize; const i=address<0xc000 ? this.bank%bankCount*bankSize+address-0x8000 : (bankCount-1)*bankSize+address-0xc000; return this.rom.prgRom[i]; } const i=address-0x8000; return this.rom.prgRom[i%this.rom.prgRom.length];} return 0;}
  write(address:number,value:number){if(address<0x2000)this.ram[address&0x7ff]=value&255; else if(address===0x4016){this.controller1.write(value); this.controller2.write(value);} else if(address>=0x8000 && this.rom.mapper===2)this.bank=value&255;}

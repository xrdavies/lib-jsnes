...
export class Nes implements CpuBus {
  ...
  step(cycles=1){if(!Number.isInteger(cycles)||cycles<1)throw new RangeError('cycles must be a positive integer'); const target=this.cycles+cycles; while(this.cycles<target){if(this.dmaStall){const used=Math.min(this.dmaStall,target-this.cycles); this.dmaStall-=used; this.cycles+=used; this.ppu.step(used*3); continue;} const used=this.cpu.step(); this.cycles+=used; this.ppu.step(used*3); for(let i=0;i<this.ppu.consumeScanlines();i++)if(this.cartridge.clockScanline())this.cpu.irq(); if(this.ppu.consumeNmi())this.cpu.nmi();} this.apu.step(cycles);}
  ...
  loadState(state: Uint8Array): void { const cartSize=20+0x2000, ppuSize=0x4000+32+256+13, apuSize=20, size=11+cartSize+ppuSize+apuSize+8+0x800; if(state.length!==size)throw new RangeError('Invalid state size'); let o=0;this.cpu.load(Array.from(state.subarray(o,o+=11)));this.cartridge.loadState(state.subarray(o,o+=cartSize));this.ppu.loadState(state.subarray(o,o+=ppuSize));this.apu.loadState(state.subarray(o,o+=apuSize));this.controller1.loadState(state.subarray(o,o+=4));this.controller2.loadState(state.subarray(o,o+=4));this.ram.set(state.subarray(o)); }

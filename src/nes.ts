...
export class Nes implements CpuBus { readonly controller1 = new Controller(); readonly ppu = new Ppu(); readonly controller2 = new Controller(); readonly rom: RomImage; get frame(): Uint32Array { return this.ppu.frame; } readonly cpu: Cpu6502; private readonly ram = new Uint8Array(0x800); private cycles = 0; private dmaStall = 0; private bank = 0;
  constructor(image: ArrayBuffer | Uint8Array) { this.rom=parseRom(image); if(this.rom.mapper!==0&&this.rom.mapper!==2) throw new Error(`Unsupported mapper: ${this.rom.mapper}`); this.ppu.setMirroring(this.rom.mirroring); this.ppu.setChr(this.rom.chrRam?new Uint8Array(0x2000):this.rom.chrRom); this.cpu=new Cpu6502(this); }
  ...
  write(address:number,value:number){if(address>=0x2000 && address<0x4000){this.ppu.writeRegister(address,value); return;} if(address<0x2000)this.ram[address&0x7ff]=value&255; else if(address===0x4016){this.controller1.write(value); this.controller2.write(value);} else if(address===0x4014){this.dmaStall += 513; const page=(value&255)<<8; const bytes=new Uint8Array(256); for(let i=0;i<256;i++)bytes[i]=this.read(page+i); this.ppu.dma(bytes);} else if(address>=0x8000 && this.rom.mapper===2)this.bank=value&255;}
  reset(){this.ram.fill(0); this.bank=0; this.dmaStall=0; this.ppu.frame.fill(0xff000000); this.cpu.reset(); this.cycles=0;}

...
export class Apu {
  ...
  loadState(v:Uint8Array):void { if(v.length!==20)throw new RangeError('Invalid APU state'); this.samples=[]; this.frac=0;this.pulse[0].regs.set(v.subarray(0,4));this.pulse[1].regs.set(v.subarray(4,8));this.noise.regs.set(v.subarray(8,12));this.pulse[0].phase=v[12];this.pulse[1].phase=v[13];this.noise.shift=v[14]|(v[15]<<8);this.pulse[0].enabled=!!v[16];this.pulse[1].enabled=!!v[17];this.noise.enabled=!!v[18];this.frame=v[19];}
  readStatus():number { return (this.pulse[0].length?1:0)|(this.pulse[1].length?2:0)|(this.triangle.length?4:0)|(this.noise.length?8:0); }
  step(cycles:number):void {for(let i=0;i<cycles;i++){for(const p of this.pulse)p.step();this.triangle.step();this.noise.step();if(++this.frame>=7457){this.frame=0;for(const p of this.pulse)p.clockLength();this.triangle.clockLength();this.noise.clockLength();}this.frac+=SAMPLE_HZ;if(this.frac>=CPU_HZ){this.frac-=CPU_HZ;if(this.samples.length>=SAMPLE_HZ*2)this.samples.shift(); this.samples.push((this.pulse[0].sample()+this.pulse[1].sample()+this.triangle.sample()+this.noise.sample())*320-4096);}}}

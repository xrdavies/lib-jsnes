...
export class Apu { private readonly pulse=[new Pulse(),new Pulse()]; private readonly noise=new Noise(); private readonly triangle=new Triangle(); private frac=0; private frame=0; private samples:number[]=[];
  ...
  saveState():Uint8Array { const out=new Uint8Array(4*2+4+8); out.set(this.pulse[0].regs,0);out.set(this.pulse[1].regs,4);out.set(this.noise.regs,8);out[12]=this.pulse[0].phase;out[13]=this.pulse[1].phase;out[14]=this.noise.shift&255;out[15]=this.noise.shift>>>8;out[16]=this.pulse[0].enabled?1:0;out[17]=this.pulse[1].enabled?1:0;out[18]=this.noise.enabled?1:0;out[19]=this.frame&255; return out; }
  loadState(v:Uint8Array):void { if(v.length!==20)throw new RangeError('Invalid APU state');this.pulse[0].regs.set(v.subarray(0,4));this.pulse[1].regs.set(v.subarray(4,8));this.noise.regs.set(v.subarray(8,12));this.pulse[0].phase=v[12];this.pulse[1].phase=v[13];this.noise.shift=v[14]|(v[15]<<8);this.pulse[0].enabled=!!v[16];this.pulse[1].enabled=!!v[17];this.noise.enabled=!!v[18];this.frame=v[19];}
  ...
  step(cycles:number):void {for(let i=0;i<cycles;i++){for(const p of this.pulse)p.step();this.triangle.step();this.noise.step();if(++this.frame>=7457){this.frame=0;for(const p of this.pulse)p.clockLength();this.triangle.clockLength();this.noise.clockLength();}this.frac+=SAMPLE_HZ;if(this.frac>=CPU_HZ){this.frac-=CPU_HZ;this.samples.push((this.pulse[0].sample()+this.pulse[1].sample()+this.triangle.sample()+this.noise.sample())*320-4096);}}}

export class Ppu {
  ...
  private ctrl=0; private mask=0; private scrollX=0; private scrollY=0; private status=0xa0; private oamAddr=0; private addr=0; private latch=false; private data=0; scanline=0; dot=0;
  ...
  private renderBackground():void { for(let y=0;y<240;y++)for(let x=0;x<256;x++){const sx=(x+this.scrollX)&255, sy=(y+this.scrollY)&239, tile=this.vram[0x2000+((sy>>>3)*32)+(sx>>>3)]; const base=(this.ctrl&16?0x1000:0)+tile*16, row=sy&7, col=sx&7, lo=this.chr[base+row]??0, hi=this.chr[base+row+8]??0, c=((lo>>(7-col))&1)|(((hi>>(7-col))&1)<<1); this.frame[y*256+x]=0xff000000|(c*0x555555);}}

export class Ppu {
  readonly vram = new Uint8Array(0x4000); readonly oam = new Uint8Array(256); readonly frame = new Uint32Array(256*240);
  ...
  readRegister(reg:number):number { switch(reg&7){case 2: {const v=this.status; this.status&=0x7f; this.latch=false; return v;} case 4:return this.oam[this.oamAddr]; case 7:{const v=this.data; this.data=this.vram[this.addr]; this.addr=(this.addr+(this.ctrl&4?32:1))&0x3fff; return v;} default:return 0;} }
  writeRegister(reg:number,value:number):void {value&=255; switch(reg&7){case 0:this.ctrl=value;break;case 1:this.mask=value;break;case 3:this.oamAddr=value;break;case 4:this.oam[this.oamAddr++]=value;break;case 5:this.latch=!this.latch;break;case 6:if(!this.latch)this.addr=(value&0x3f)<<8;else this.addr=(this.addr&0x3f00)|value;this.latch=!this.latch;break;case 7:this.vram[this.addr]=value;this.addr=(this.addr+(this.ctrl&4?32:1))&0x3fff;}}
